...
-- Web build intentionally disables translators/processors that write desktop
-- files or execute OS commands. Keep a no-op binding so dependent schemas can
-- still be deployed if selected manually.
function en_custom_translator(input, seg)
end
...
-- 只启用静态候选调整过滤；网页端禁用原方案里写本机文件的 processor。
